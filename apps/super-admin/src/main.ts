...
async function bootstrap() {
  ...
  fastifyAdapter.get('/', (req: FastifyRequest, res: FastifyReply) => {
    res.code(200);
    res.send({
      message: 'Welcome to LMS SuperAdmin',
    });
  });
  ...
  fastify.after(() => {
    fastify.addHook(
      ...
      (request, reply, done) => {
        ...
        if (!request.raw.url.startsWith('/api-docs')) {
          return done();
        }
  ...
  const options = new DocumentBuilder()
    .setTitle('LMS API for SuperAdmin')
    .setDescription('API documentation for LMS SuperAdmin')
    .setVersion('1.0')
    .addBearerAuth()
    .build();
  ...
  const port = process.env.PORT_SUPERADMIN || 80;
  try {
